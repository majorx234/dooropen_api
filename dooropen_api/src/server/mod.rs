...
mod paths {
    ...
    lazy_static! {
        pub static ref GLOBAL_REGEX_SET: regex::RegexSet =
            regex::RegexSet::new(vec![r"^/v1.0/door_status$", r"^/v1.0/ping$"])
            ...
            match method {
                ...
                hyper::Method::GET if path.matched(paths::ID_DOOR_STATUS) => {
                    ...
                    match result {
                        Ok(rsp) => match rsp {
                            ...
                            DoorStatusResponse::TheHTTP(body) => {
                ...
                hyper::Method::GET if path.matched(paths::ID_PING) => {
                    ...
                    match result {
                        Ok(rsp) => {
                            match rsp {
                                PingResponse::Success(body) => {
                                    *response.status_mut() = StatusCode::from_u16(200)
                                        .expect("Unable to turn 200 into a StatusCode");
                                    response.headers_mut().insert(
                                        CONTENT_TYPE,
                                        HeaderValue::from_str("application/json").expect(
                                            "Unable to create Content-Type header for PING_SUCCESS",
                                        ),
                                    );
                                    let body = serde_json::to_string(&body)
                                        .expect("impossible to fail to serialize");
