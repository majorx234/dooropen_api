...
use crate::{Api, DoorStatusResponse, PingResponse};
    ...
    C: Has<XSpanIdString> + Clone + Send + Sync + 'static,
{
    ...
    async fn door_status(&self, context: &C) -> Result<DoorStatusResponse, ApiError> {
        ...
        let mut uri = format!("{}/v1.0/door_status", self.base_path);
        ...
        match response.status().as_u16() {
            200 => {
                ...
                let body = serde_json::from_str::<models::DoorStatus>(body).map_err(|e| {
                    ApiError(format!("Response body did not match the schema: {}", e))
                })?;
                Ok(DoorStatusResponse::Success(body))
            }
            403 => {
                let body = response.into_body();
                let body = body
                ...
                Ok(DoorStatusResponse::TheHTTP(body))
            }
            code => {
    ...
    async fn ping(&self, context: &C) -> Result<PingResponse, ApiError> {
        let mut client_service = self.client_service.clone();
        let mut uri = format!("{}/v1.0/ping", self.base_path);
